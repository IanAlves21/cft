...
- https://www.postgresql.org/docs/6.4/sql-createtable.htm

#### Para Instalar HomeBrew (Gerenciador de Pacotes para MacOS)
```rconsole
$ ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"
```
#### Para Instalação e Configuração Inicial do Banco Postgres
```rconsole
...
postgres$ CREATE DATABASE db_cft;
```
...
### Script Para Criação da Tabela Item
...
### Script Para Criação da Tabela Lista
...
(
  id SERIAL PRIMARY KEY,
  active BOOLEAN,
  user_id INTEGER REFERENCES users(id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE
)
WITH (
  OIDS=FALSE
);
...
### Script Para Criação da Tabela de Relacionamento Lista_Item
...
(
  id SERIAL PRIMARY KEY,
  list_id INTEGER REFERENCES list (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE,
  item_id INTEGER REFERENCES item (id) MATCH SIMPLE ON UPDATE NO ACTION ON DELETE CASCADE,
  quantity INTEGER
)
WITH (
  OIDS=FALSE
...
### Alternativamente, o banco pode ser restaurado conforme se observa a seguir: 
#### Com a ***role*** postgres e ***database*** db_cft criados anteriormente, executar os seguintes comandos:

```rconsole
$ git clone https://github.com/VitorMatheus097/CFT-Project cft-project
$ cd cft-project/backend/data
$ pg_restore -h localhost -U postgres -W -d db_cft -1 data.backup
```

### Para a Execução da API e Conexão com o Banco de Dados:
